...
NaLinha Web é uma aplicação estática (HTML/CSS/JS puro, ES modules, sem bundler/build step, sem `package.json`), publicada via GitHub Pages, que exibe dados de operação e movimentação dos ônibus da URBS (Curitiba). Todos os dados vêm de um repositório GitHub companheiro (`leandrohstein/transporteservico-urbs-data`, branch `data`) que publica o feed público da URBS como arquivos JSON estáticos (snapshots de veículos por minuto, lista de linhas, tipos de veículo, geoJson de trajeto por linha). Este app é só um consumidor estático desses arquivos via `raw.githubusercontent.com` — não existe backend próprio; "sincronizar" sempre significa "buscar os minutos faltantes nesses JSONs e cachear em IndexedDB".
