...
NaLinha Web é uma aplicação estática (HTML/CSS/JS puro, ES modules, sem bundler/build step, sem `package.json`), publicada via GitHub Pages, que exibe dados de operação e movimentação dos ônibus da URBS (Curitiba). Todos os dados vêm de repositórios GitHub companheiros da organização `nalinha-app` que publicam o feed público da URBS como arquivos JSON estáticos (snapshots de veículos por minuto, lista de linhas, tipos de veículo, geoJson de trajeto por linha). Os dumps de veículos são segregados por ano em repositórios separados (`transporteservico-urbs-data-2023`, `-2024`, `-2025`, `-2026`, ...) — a URL usada depende do ano da data buscada (`buildVeiculosBaseUrl` em `services/vehicleDataService.js`). Cada repositório de ano tem duas branches relevantes: `data` (arquivos por minuto originais, e onde ficam linhas/tipos de veículo/geoJson) e `consolidated` (arquivos consolidados por hora, `HH.json` — ver `docs/formato-all-json.md`). Linhas, tipos de veículo e geoJson de trajeto não são segregados por ano e ficam sempre no repositório mais recente (`transporteservico-urbs-data-2026`). Este app é só um consumidor estático desses arquivos via `raw.githubusercontent.com` — não existe backend próprio; "sincronizar" sempre significa "buscar os minutos faltantes nesses JSONs e cachear em IndexedDB".
